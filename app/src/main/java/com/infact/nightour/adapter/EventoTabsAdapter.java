...
import com.infact.nightour.fragment.ProgramacaoEventoFragment;
 ...
 */
public class EventoTabsAdapter extends FragmentPagerAdapter {

    private Context context;

    public EventoTabsAdapter(Context context, FragmentManager fm){
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        if(position == 0){
            return new ProgramacaoEventoFragment();
        }else if(position == 1){
    ...
    public CharSequence getPageTitle(int position) {
        if(position == 0){
            return context.getString(R.string.programação);
        }else if(position == 1){
